...
## 🔴 CRITICAL — URL Slug Changes Required

Developer must change these URL slugs before launch. Our SEO-optimized slugs include the area + building name which helps Google rank for location keywords.

| Current Dev Slug | Change To (Final) |
|---|---|
...
All config files (meta tags, schemas, sitemap, alt tags) use the FINAL slugs above. Developer must update their Next.js routing to match.

---

## 🔴 CRITICAL — Canonical Tag Implementation

Every page MUST have a `<link rel="canonical">` tag. This prevents Google from splitting ranking between duplicate URLs (with/without trailing slash, query params, etc.)

**If using our meta-tags.js helper (recommended):**
```javascript
// app/about-us/page.js
import { getMetadata } from '@/seo/meta/meta-tags'
export const metadata = getMetadata('/about-us')
// This automatically adds: <link rel="canonical" href="https://workez.in/about-us">
```

**If implementing manually in `<Head>`:**
```html
<link rel="canonical" href="https://workez.in/about-us" />
```

Canonical URLs for all 28 pages are in `meta/workez-meta-tags.json` under the `canonical` field.

**Rules:**
- Always use `https://` (not http)
- Always use `workez.in` (not www.workez.in or dev.workez.in)
- No trailing slash (unless homepage)
- No query parameters
